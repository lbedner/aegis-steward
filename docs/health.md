...
aegis-steward includes comprehensive health monitoring capabilities through both API endpoints and CLI commands.
...
aegis-steward provides a built-in CLI for health monitoring:

### Basic Health Check
```bash
aegis-steward health status
```

**Output:**
```
aegis-steward - System Status: HEALTHY
...
### Detailed Health Check
```bash
aegis-steward health status --detailed
```
...
### JSON Output
```bash
aegis-steward health status --json
```
...
- **Job History**: Recent job execution status
...
aegis-steward health check --debug
```
