...
class TestTheWriteHalfDoesNotPileUp:
    """One ``FinanceImportBatchRow`` per row, added and never flushed.

    ``create_transaction`` flushes, so an INSERT-heavy import drains the
    session as a side effect and looks fine. A DUPLICATE-heavy one - a
    re-import of a file the ledger already holds, which is the common
    case - takes the ``continue`` before any of that, so nothing flushes
    and nothing queries, which means no autoflush either. Measured
    2026-09-19: 18,607 rows held 51.3 MiB unflushed, 2.0 MiB with a
    periodic flush.

    Guards the count of PENDING objects, which is the thing that grew;
    ``test_ingest_query_count_is_flat_in_row_count`` guards queries and
    ``TestPlanningDoesNotHydrateTheLedger`` guards the read half.
    """

    @pytest.mark.asyncio
    async def test_pending_rows_are_bounded_not_the_whole_file(
        self,
        svc: FinanceService,
        async_db_session: AsyncSession,
        csv_profiles: None,
        ...
        monkeypatch.setattr(imports, "FLUSH_EVERY_ROWS", 4)
        account = await _account(async_db_session)

        def _rows() -> list[ParsedTransaction]:
            return [
                ParsedTransaction(
                    date=date(2026, 3, 1),
                    amount=-100 - i,
        ...
        # First pass puts them in the ledger; the second is the re-import.
        await imports.ingest_transactions(
            async_db_session,
            owner_user_id=1,
            source_type="ofx",
        ...
        pending_at_each_flush: list[int] = []

        def _on_flush(session: Any, ctx: Any, instances: Any) -> None:
            pending_at_each_flush.append(len(session.new))

        event.listen(SyncSession, "before_flush", _on_flush)
        try:
            result = await imports.ingest_transactions(
                async_db_session,
                owner_user_id=1,
            ...
            event.remove(SyncSession, "before_flush", _on_flush)

        # The case that pays for this: every row a duplicate, so the loop
        # never reaches create_transaction and never queries.
        assert result.rows_duplicate == 12 and result.rows_inserted == 0

        # +2 slack: the batch row itself, and the last partial chunk.
        assert max(pending_at_each_flush) <= 6, (
            f"the whole file accumulated before a flush: {pending_at_each_flush}"
        )


class TestTheLaneShapesNameRealColumns:
    """``lane_columns`` derives the select from the shape's field names, so
    the order cannot drift between the two - but only while every field
    still names a column. A renamed model attribute would otherwise fail
    at query time, in a worker, on a file."""

    def test_every_lane_field_is_a_transaction_column(self) -> None:
        from app.services.finance.adapters.importers.base import (
            DeletedLaneRow,
            LaneRow,
            lane_columns,
        ...
        for shape in (LaneRow, DeletedLaneRow):
            columns = lane_columns(shape)
            assert len(columns) == len(shape._fields)
            # Positional construction is only safe if position N of the
            # select is field N of the shape.
            assert [c.key for c in columns] == list(shape._fields)
