...
async def _backend_component_health() -> ComponentStatus:
    ...
    try:
        ...
        # Local request-metrics summary (ephemeral, in-memory). The
        # service is always-on and singleton; populated by the
        # performance middleware. Empty until the first request lands.
        # ``performance`` is the cross-endpoint roll-up shown on the
        # Server card; ``performance_endpoints`` is the per-route
        # detail consumed by the modal's Performance tab.
        try:
            from app.components.backend.middleware.performance import (
                metrics_service,
            )

            performance_summary = metrics_service.get_summary_stats()
            performance_endpoints = metrics_service.get_all_metrics()
        except Exception:
            performance_summary = {}
...
async def _web_frontend_component_health() -> ComponentStatus:
    """
    ...
    try:
        ...
        return ComponentStatus(
            name="web_frontend",
            status=ComponentStatusType.UNHEALTHY,
            message="Web frontend component not found",
            response_time_ms=None,
            metadata={
                "type": "component_check",
                "error": "import_error",
        ...
        return ComponentStatus(
            name="web_frontend",
            status=ComponentStatusType.UNHEALTHY,
            message=f"Web frontend component error: {str(e)}",
            response_time_ms=None,
            metadata={
                "type": "component_check",
                "error": "unexpected_error",
...
async def startup_hook() -> None:
    ...
    register_health_check("web_frontend", _web_frontend_component_health)
