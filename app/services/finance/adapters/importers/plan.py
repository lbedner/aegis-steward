"""Deciding what an import WOULD do, without writing anything.

A pure read, kept in its own module because that purity is the whole
contract: ``preview_file`` returns a plan untouched and
``ingest_transactions`` executes one, so what the preview shows is by
construction what a commit does rather than a parallel re-implementation
that can drift.

Three matching lanes run per row, most authoritative first: a provider id
...
or re-categorized charge plans as an update to the existing row instead
of a second copy of the same money.
"""
...
async def plan_transactions(
    ...
    *,
    ...
    if touched_account_ids:
        ...
        # same unchanged import got dearer every month until it died; see
        # ``LaneRow``. The entities the plan genuinely needs - the rows it
        # means to EDIT - are fetched by id below, once they are known.
        dedup_rows = await queries.live_transaction_lane_rows(db, touched_account_ids)
    ...
    if touched_account_ids:
        deleted_rows = await queries.deleted_transaction_lane_rows(
    ...
    # The only rows the plan needs as models. ``_plan_edit`` reads the
    # label fields off them and the commit MUTATES those very objects, so
    # these have to be real; nothing else does.
    #
    # Exact, not a superset: ``_edit_target`` returns a row only where
    # ``core_incoming[key] == 1``, so no two incoming rows can ever claim
    # the same key, and the loop's own ``claimed_existing.add`` below
    # cannot change any answer given here. On a re-import of a file the
    # ledger already holds, every row matches lane 1 or 2 and this set is
    # empty - which is the case that used to build 18,571 of them.
    edit_targets = {
        target
        for candidate in postable
        if (candidate_account := account_by_key.get(candidate.account_key)) is not None
        and (target := _edit_target(candidate_account, candidate)) is not None
    }
    existing_by_id: dict[int, FinanceTransaction] = await service.transactions_by_ids(
        sorted(edit_targets)
    )
