...
async def ingest_transactions(
    ...
    *,
    ...
    if on_label is not None:
        ...
        # memory go (123 MiB and most of the wall clock for an 18,607-row
        # file, measured 2026-09-19) and it runs before a single row is
        # written. A label that arrives afterwards describes a wait the
        # user has already spent staring at the previous one.
        await on_label(f"Checking {len(parsed):,} rows against your ledger...")
    ...
    for done, row in enumerate(plan.rows, 1):
        ...
        if done % PROGRESS_EVERY_ROWS == 0:
            ...
            if on_label is not None:
                # Counts, not a percentage: "4,000 of 18,607" says how much
                # is left AND that the numbers are moving, and the two the
                # user cares about are what landed and what was already here.
                await on_label(
    ...
    if inserted or updated:
        ...
        if on_label is not None:
            ...
            # insights over the whole ledger) and the row counter has
            # stopped by now, so silence here reads as a stall.
            await on_label("Reconciling transfers, bills and insights...")
        logger.info(
